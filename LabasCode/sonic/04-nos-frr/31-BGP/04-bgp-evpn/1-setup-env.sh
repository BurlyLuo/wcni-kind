#1. EVPN Route Type:
# type1 and type4 用作EVPN MH
# type2 用作通告MAC/IP/ARP等信息的
# type3 用作VTEP设备间的发现和VxLAN Tunnel的建立的
# type5 用作引入外部路由的

#2. 关于BGP路由表,系统路由表，FIB表解释[以该topo下的形式：leaf1上查看10.1.5.254/32作为demo]
# 1.list bgp表，route表，fib表：
# 1.1：bgp表[Status codes:  s suppressed, d damped, h history, u unsorted, * valid, > best, = multipath,]
# leaf1# show ip bgp vrf red | include 10.1.5.254/32
#  *>  10.1.5.254/32    100.65.0.2<                            0 64612 64513 i   [在bgp表中表明是* valid, > best的]
# leaf1# 

# 1.2：route表：[       > - selected route, * - FIB route, q - queued, r - rejected, b - backup]
# leaf1# show ip route  vrf red | include 10.1.5.254/32   
# B   10.1.5.254/32 [20/0] via 100.65.0.2, br100 onlink, weight 1, 00:35:31 [从bgp过来的]
# L * 10.1.5.254/32 is directly connected, br5, weight 1, 00:35:33 [本地Local的路由]
# L>* 10.1.5.254/32 is directly connected, br5, weight 1, 00:35:35 [本地Local的路由被优选]

# 1.3：[       > - selected route, * - FIB route, q - queued, r - rejected, b - backup]
# leaf1# show ip fib vrf red | include 10.1.5.254/32      
# L>* 10.1.5.254/32 is directly connected, br5, weight 1, 00:35:41 [地Local的路由被优选]
# leaf1# 
  ...
  nodes:
    leaf1:
      ...
      exec:
        ...
        # if vlan5 and vlan8 from same interface eth2.[Add eth2.5 into br5 eth2.8 into br8]
        - ip link add link eth2 name eth2.5 type vlan id 5  
        - ip link set eth2.5 up
        - ip link set eth2.5 master br5

        - ip link add link eth3 name eth3.8 type vlan id 8
        - ip link set eth3.8 up
        - ip link set eth3.8 master br8
    ...
    leaf2:
      ...
      exec:
        ...
        # to 10.1.5.11/24 and 10.1.8.11/24 
        - ip link add link eth2 name eth2.5 type vlan id 5
        - ip link set eth2.5 up
        - ip link set eth2.5 master br5
