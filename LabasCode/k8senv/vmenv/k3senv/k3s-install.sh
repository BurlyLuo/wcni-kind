...
download_hash() {
    if [ -n "${INSTALL_K3S_PR}" ]; then
        ...
        curl -o ${TMP_ZIP} -H "Authorization: Bearer $TOKEN_REMOVED" -L ${GITHUB_PR_URL}
...
get_pr_artifact_url() {
    ...
    if [ -z "${TOKEN_REMOVED}" ]; then
        fatal "Installing PR builds requires TOKEN_REMOVED with k3s-io/k3s repo permissions"
    fi
    # GET request to the GitHub API to retrieve the latest commit SHA from the pull request
    pr_raw=$(curl -s -H "Authorization: Bearer $TOKEN_REMOVED" "$github_api_url/pulls/$INSTALL_K3S_PR")
    ...
    wf_raw=$(curl -s -H "Authorization: Bearer $TOKEN_REMOVED" "$github_api_url/commits/$commit_id/check-runs")
    ...
    artifacts=$(curl -s -H "Authorization: Bearer $TOKEN_REMOVED" "$github_api_url/actions/runs/$run_id/artifacts")
