...
cilium_ds_pod=$(
  for i in $(kubectl -nkube-system get pods -o name | grep -e '^[^-]*-[^-]*$' | awk -F "/" '{print $2}')
  do
    kubectl -nkube-system exec -it $i -c cilium-agent -- env | grep $controller_node_name>/dev/null
    if [ $? -eq 0 ]; then
      echo $i
      break
    fi
...
if ! curl -m 1 $controller_node_ip:32000 >/dev/null 2>&1; then
