...
topology:
  ...
  links:
    ...
    # spine1 <> leaf3
    # 10.1.110.2/24 <> 10.1.110.1/24
    - endpoints: ["spine1:eth3", "leaf3:eth1"]

    # spine1 <> leaf4
    # 10.1.134.2/24 <> 10.1.134.1/24
    ...
    # spine2 <> leaf3
    # 10.1.112.2/24 <> 10.1.112.1/24
    - endpoints: ["spine2:eth3", "leaf3:eth2"]

    # spine2 <> leaf4
    # 10.1.111.2/24 <> 10.1.111.1/24
    ...
    # leaf3 <> server1
    # 10.1.5.11 vlan5
    - endpoints: ["leaf3:eth4", "server1:eth2"]

    # leaf3 <> server2
    # 10.1.8.11 vlan8
    - endpoints: ["leaf3:eth4", "server2:eth2"]
    ...
    # leaf4 <> server3
    # 10.1.5.11 vlan5
    - endpoints: ["leaf2:eth3", "server3:eth2"]

    # leaf4 <> server4
    # 10.1.8.11 vlan8
    - endpoints: ["leaf2:eth4", "server4:eth2"]
EOF
...
declare -A precfg_vm=(
    ...
    ["leaf3"]="echo 'export PYTHONWARNINGS=ignore::SyntaxWarning' >> ~/.bashrc"

    ["leaf4"]="echo 'export PYTHONWARNINGS=ignore::SyntaxWarning' >> ~/.bashrc"
)
...
sudo /usr/local/bin/sonic-cfggen -d --print-data > /etc/sonic/config_db.json"
    ...
    ["leaf3"]="sudo config reload -f -y
sudo config hostname leaf3
# config sonic vlan:
sudo config vlan add 5
sudo config vlan member add -u 5 Ethernet8
sudo config vlan add 8
...
sudo config interface ip add Ethernet0 10.1.110.1/24
sudo config interface ip add Ethernet4 10.1.112.1/24
# config vxlan:
sudo config vxlan add vtep 3.3.3.3
sudo config vxlan evpn_nvo add nvo vtep
sudo config vxlan map add vtep 5 5000
sudo config vxlan map add vtep 8 8000
...
sudo /usr/local/bin/sonic-cfggen -d --print-data > /etc/sonic/config_db.json"
    ...
    ["leaf4"]="sudo config reload -f -y
sudo config hostname leaf4
# config sonic vlan:
sudo config vlan add 5
sudo config vlan member add -u 5 Ethernet8
sudo config vlan add 8
...
# config ip address:
sudo config loopback add Loopback0
sudo config interface ip add Loopback0 9.9.9.9/32
sudo config interface ip add Ethernet0 10.1.134.1/24
sudo config interface ip add Ethernet4 10.1.111.1/24
# config vxlan:
sudo config vxlan add vtep 9.9.9.9
sudo config vxlan evpn_nvo add nvo vtep
sudo config vxlan map add vtep 5 5000
sudo config vxlan map add vtep 8 8000
